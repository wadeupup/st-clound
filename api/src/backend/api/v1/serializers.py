...
from api.v1.serializer_utils.check_metadata_i18n import get_localized_check_metadata
...
class FindingSerializer(RLSSerializer):
    ...
    check_metadata = serializers.SerializerMethodField()
    ...
    def get_check_metadata(self, obj):
        request = self.context.get("request")
        accept_language = request.headers.get("Accept-Language") if request else None
        provider = getattr(getattr(obj.scan, "provider", None), "provider", "")
        localized_metadata = get_localized_check_metadata(
            provider=provider.lower(),
            check_id=obj.check_id,
            accept_language=accept_language,
        )
        return localized_metadata or obj.check_metadata
