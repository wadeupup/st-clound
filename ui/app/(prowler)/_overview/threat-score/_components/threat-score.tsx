...
function convertSectionScoresToTooltipData(
  ...
  return Object.entries(sectionScores).map(([name, value]) => {
    // Determine color based on the same ranges as config
    const threatLevel = getThreatLevel(value, config);
    const color = config[threatLevel].color;
...
export function ThreatScore({
  ...
  const threatLevelConfig = getThreatLevelConfig(t);
  const threatLevel = getThreatLevel(displayScore, threatLevelConfig);
  ...
  return (
    ...
    >
      <CardHeader>
        <CardTitle>{t.overview.threatScore.title}</CardTitle>
      </CardHeader>
        ...
        <div className="relative mx-auto h-[172px] w-full max-w-[250px]">
          <div className="absolute top-0 left-1/2 z-1 w-full -translate-x-1/2">
            <RadialChart
              ...
              label={t.overview.threatScore.score}
              color={config.color}
