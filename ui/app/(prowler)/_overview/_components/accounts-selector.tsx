...
export function AccountsSelector({ providers }: AccountsSelectorProps) {
  ...
  const filterDescription =
    selectedTypesList.length > 0
      ? `${t.overview.accountsSelector.showingAccountsFor} ${selectedTypesList.join(", ")} ${t.overview.accountsSelector.providers}`
      : t.overview.accountsSelector.allConnectedAccounts;
        ...
        <MultiSelectContent search={false}>
          {visibleProviders.length > 0 ? (
            <>
              ...
              >
                {t.overview.accountsSelector.selectAll}
              </div>
            ...
            <div className="px-3 py-2 text-sm text-slate-500 dark:text-slate-400">
              {selectedTypesList.length > 0
                ? t.overview.accountsSelector.noAccountsForProviders
                : t.overview.accountsSelector.noAccountsAvailable}
            </div>
          )}
        </MultiSelectContent>
      </MultiSelect>
