...
const buildCustomBreadcrumbs = (
  ...
  onBackToResource: (() => void) | undefined,
  t: ReturnType<typeof useI18n>["t"],
): CustomBreadcrumbItem[] => {
  const breadcrumbs: CustomBreadcrumbItem[] = [
    {
      name: t.resources.resourceDetail.breadcrumbResourceDetails,
...
export const ResourceDetail = ({
    ...
    return (
      <div className="flex min-h-96 flex-col items-center justify-center gap-4 rounded-lg p-8">
        ...
        <p className="dark:text-prowler-theme-pale/80 text-sm text-gray-600">
          {t.resources.resourceDetail.loadingResourceDetails}
        </p>
      </div>
    );
  }
  ...
  if (selectedFindingId) {
    const findingTitle =
      ...
      t.resources.resourceDetail.findingDetail;
      ...
      <Card variant="base" padding="lg">
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <InfoField label={t.resources.resourceDetail.resourceName}>
              {renderValue(attributes.name)}
            </InfoField>
            <InfoField label={t.resources.resourceDetail.resourceType}>
              {renderValue(attributes.type)}
            </InfoField>
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <InfoField label={t.resources.resourceDetail.service}>
              {renderValue(attributes.service)}
            </InfoField>
            <InfoField label={t.resources.resourceDetail.region}>
              {renderValue(attributes.region)}
            </InfoField>
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <InfoField label={t.resources.resourceDetail.partition}>
              {renderValue(attributes.partition)}
            </InfoField>
            <InfoField label={t.resources.resourceDetail.details}>
              {renderValue(attributes.details)}
            </InfoField>
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <InfoField label={t.resources.resourceDetail.createdAt}>
              <DateWithTime inline dateTime={attributes.inserted_at} />
            </InfoField>
            <InfoField label={t.resources.resourceDetail.lastUpdated}>
          ...
          {resourceTags && Object.entries(resourceTags).length > 0 ? (
            <div className="flex flex-col gap-4">
              <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400">
                {t.resources.resourceDetail.tags}
              </h4>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <CardContent>
          {findingsLoading ? (
            <div className="flex items-center justify-center gap-2 py-8">
              <Spinner size="sm" />
              <p className="dark:text-prowler-theme-pale/80 text-sm text-gray-600">
                {t.resources.resourceDetail.loadingFindings}
              </p>
            </div>
            ...
            <p className="dark:text-prowler-theme-pale/80 text-gray-600">
              {t.resources.resourceDetail.noFailedFindings}
            </p>
          )}
        </CardContent>
      </Card>
