    ...
    <Card variant="base" padding="none" className="rounded-xl border border-slate-200 bg-white/80 backdrop-blur-sm shadow-lg dark:border-slate-800 dark:bg-slate-900/80 p-6">
      <CardContent>
        <div className="mb-6 flex flex-col gap-1">
          <h4 className="text-lg font-bold text-slate-900 dark:text-slate-100">{t.profile.organizations}</h4>
          <p className="text-xs text-slate-600 dark:text-slate-400">
            {t.profile.organizationsAssociated}
          </p>
        </div>
        {memberships.length === 0 ? (
          <div className="text-sm text-slate-600 dark:text-slate-400">{t.profile.noMembershipsFound}</div>
        ) : (
